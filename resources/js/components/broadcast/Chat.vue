   ...
   <!-- ----------------------------------buuton ------------------------------ -->
      ...
      <!-- ---------------------------------------------------------------------- -->
        <div id="chatbody" ref="chatbody" class="chatbody col-12  pl-3 "
          style="height: 440px; overflow-y: scroll; overflow-x:hidden;scroll-behavior: smooth;">
          
          <!-- public---------- -->
  <div v-show="msgdiv">
        <div  id="chatmessage" v-for="(message,index) in messages" :key="index" class="d-flex m-1"style="overflow-wrap: break-word;word-wrap: break-word;">
...
export default {
  ...
  data() {
    return {
             currentroom:this.$store.getters.user.getroom.name,
